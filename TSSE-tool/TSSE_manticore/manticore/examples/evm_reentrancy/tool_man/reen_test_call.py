        ...
        if state.platform.current_transaction.sort != "CREATE":
            if instruction.semantics == "CALL":
                ...
                if issymbolic(dest_address):
                    ...
                    (caller_migrated,address_migrated,value_migrated,data_migrated,gas_migrated,price_migrated) = m._migrate_tx_expressions(state, msg_sender, dest_address, sent_value, calldata)
                    # world.start_transaction(sort='CALL',address=address_migrated,price=price_migrated,data=data_migrated,caller=msg_sender,value=value_migrated,gas=gas_migrated)
            # if instruction.semantics == "CALLDATACOPY":
            #     print("进入库函数！！！")
            #     # world.current_vm.pc = self._Dest_loc
            #     (caller_migrated,address_migrated,value_migrated,data_migrated,gas_migrated,price_migrated) = m._migrate_tx_expressions(state, msg_sender, dest_address, sent_value, calldata)
            #     world.start_transaction(sort='CALL',address=curr_tx.address,price=price_migrated,data=data_migrated,caller=msg_sender,value=value_migrated,gas=gas_migrated)


            if instruction.semantics == "STOP":
                print("STOPSTOPSTOP进入库函数！！！")
                # (caller_migrated,address_migrated,value_migrated,data_migrated,gas_migrated,price_migrated) = m._migrate_tx_expressions(state, msg_sender, dest_address, sent_value, calldata)
        ...
        if is_exis:
            if len(state.platform.transactions) > 2:
                ...
                # if m.generate_testcase(state, "maybe reentrancy?", only_if=OR(AND(funcd == reentry_string,operators.UGT(tvaule, 0)),AND(funcd == reentry_string,balance2 != 1000))):
                    ...
                    newName="/data/home/wangzexu/manticore_pro/manticore-0.3.0/examples/evm_reentrancy/"+fname
